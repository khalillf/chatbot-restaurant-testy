...
<form method="post">
    ...
    <input class= "po" type="text" name="client_number" placeholder="Enter id de client">
...
if (isset($_POST["show_all_reservations"])) {
    ...
    $query = "SELECT * FROM il288 ORDER BY id DESC";
    ...
    $query = "SELECT * FROM il288 WHERE id = '$id' or date = '$filterDate'";
} else if (isset($_POST["show_current_day_reservations"])) {
    // Fetch reservations for the current date
    $currentDate = date("Y-m-d");
    $query = "SELECT * FROM il288 WHERE date = '$currentDate' ORDER BY id DESC";
} else {
    // Fetch reservations for the current date
    $currentDate = date("Y-m-d");
    $query = "SELECT * FROM il288 WHERE date = '$currentDate' ORDER BY id DESC";
...
<script>
    // Fonction pour envoyer une requête AJAX et supprimer une colonne
    function deleteColumn(id) {
        if (confirm("Êtes-vous sûr de vouloir supprimer cette colonne ?")) {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    // Rechargez la page après la suppression de la colonne
                    location.reload();
                }
            };
            xhttp.open("POST", "delete_column3.php", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xhttp.send("id=" + id);
        }
    }

    // Attachez un gestionnaire d'événements au clic sur les boutons supprimer
